import slm.base.{LinearMap, Looping, Module, PullArrayOps, Ring, SemiRing}

import scala.reflect.ClassTag

package object slm {
  implicit def array_ops[T: ClassTag]: PullArrayArrayOps[T] = new PullArrayArrayOps[T]

  def toString[T](expr: T)(implicit env: Printable[T]): String = env.toString(expr)

  def vector2(x: Float, y: Float): Array[Float] = Array(x, y)
  def vector3(x: Float, y: Float, z: Float): Array[Float] = Array(x, y, z)
  def vector4(x: Float, y: Float, z: Float, w: Float): Array[Float] = Array(x, y, z, w)
